...
for prec in recombinations:
	...
	FILE = open("results-rec_1_"+str(prec)+"-best.txt",'w')
	filename = "results-rec_1_"+str(prec)+".csv"
...
for pgene in pgenes:
	...
	FILE = open("results-rec_1_"+str(pgene)+"-best.txt",'w')
	filename = "results-rec_1_"+str(pgene)+".csv"
	excelWriter = csv.writer(open(filename, 'wb'))
	excelWriter.writerow(('Melhor tempo real: ', str(bestRealFitness) ))
	FILE.write("Real Curve points:\n"+str(points)+"\nFitness: "+str(bestRealFitness)+"\n\n")
